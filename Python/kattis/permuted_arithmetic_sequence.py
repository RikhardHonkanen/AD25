...
def main(input):  
    clean = [' '.join(s.split()[1:]) for s in input if len(s.split()) > 1]
    num_arrays = []
    for s in clean:
        numbers = [int(x) for x in s.split()]
        num_arrays.append(numbers)
        
    print("numarrays: ", num_arrays)
    ...
    for arr in num_arrays:
        ...
        if is_arithmetic(arr_set_sorted):
            print(arr, "can be reordered to arithmetic:", arr_set_sorted)
        else:
