...
void test_led_driver_initialization(void)
{
    led_driver_config_t config = {
        .pin.num = LED_PIN,
        .pin.mode = BSP_PIN_INPUT_OUTPUT,
        .get_state = bsp_pin_read,
        .set_state = bsp_pin_write,
        .set_mode = bsp_pin_config};

    TEST_ASSERT_FALSE(led_driver_set_state(1));

    bsp_set_config_status(false);
    TEST_ASSERT_FALSE(led_driver_init(&config));
    ...
    TEST_ASSERT_TRUE(led_driver_init(&config));
